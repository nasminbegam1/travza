...
$(function() {
	...
	$("#closed_holidays").datepicker({
		dateFormat: 'mm-dd-yy',
		minDate: 0,
	});
	$("#exp_date_bucket").datepicker({
		dateFormat: 'mm-dd-yy',
		minDate: 0,
	});
	$("#exp_date_page").datepicker({
		dateFormat: 'mm-dd-yy',
		minDate: 0,
	});
	$("#dealForm").validate({
	...
	$.validator.addMethod("offerAmount",function ()
	{
	    ...
	    if (parseInt(custom_discount) > parseInt(offer) ) {
		offerAmt = false;
		return false;
	    }
	    return offerAmt;
	...
	$('.applyPromoCode').click(function(){
		...
		if (click_count == 0 || (prev_promocode == '' || (prev_promocode != promocode)) ) {
			$.ajax({			
				...
				data: { 'promocode': promocode , "total_cost" : total_cost, "p_type" : p_type},
				success: function(msg){
