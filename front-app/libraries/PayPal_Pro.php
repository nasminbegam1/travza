<?php
/**
 * Angell EYE PayPal Pro API Class
 ...
 * 
 * PayPal NVP API:  https://cms.paypal.com/cms_content/US/en_US/files/developer/PP_NVPAPI_DeveloperGuide.pdf
 * PayPal Account Authentication: https://www.paypal-labs.com/files/PayPal_Authentication_Service.pdf
 * PayPal Permissions: https://www.paypal-labs.com/files/PayPal_Permission_Service.pdf
 * 
 * Every PayPal API method available in the documentation links above is included in this library.  
 * The controller file has a method for every single call you can make to PayPal.
 * Each controller method has a "template" for every single field available to that call.  
 * Simply populate the method fields and pass it into the library accordingly to communicate with PayPal.
 * 
 * SAMPLES
 * The controller has 2 methods configured for demo purposes and there are 2 views that coincide with these methods.
 *			- Do_direct_payment_demo()
 *			- Get_balance()
 * You can run these methods and review their code structure and get a quick understanding of how this library works.
 *
 * @package			Angell_EYE_PayPal_Pro_Class
 * @author			Andrew K. Angell
 * @copyright		Copyright (c) 2011 Angell EYE, LLC
 * @license			http://www.angelleye.com/paypal-pro-class/license
...
class PayPal_Pro
{
	 ...
	 * @return	void
	 */
	function PayPal_Pro($DataArray = array())
	{
		...
		$this -> APIVersion = isset($DataArray['APIVersion']) && $DataArray['APIVersion'] != '' ? $DataArray['APIVersion'] : '';
	 ...
	 * @return	string
	 */
	function GetStateName($StateOrProvinceCode)
	{
		$States = array_flip($this -> States);
		return $States[$StateOrProvinceCode];
	}
