	 ...
	 <div class="modal-dialog">
	     <div class="modal-content">
		  <div class="modal-header">
			   <h2>Flyer Details</h2> <a href="javascript:void(0);" id="print_flyer" class="printlink" style="display: none;"></a> 
...
draggable: false,
id: '<?php echo $ol['bucket_id'];?>',
content: '<div class="map-bg" id="ownmap_<?php echo $ol['bucket_id'];?>" onclick="ownpopUpshow(<?php echo $ol['bucket_id']?>);"><img class="shw"  src="<?php echo $profile_image ?>" style="height:40px;width:40px; border-radius:50%;"/></div><div class="owninfo_<?php echo $ol['bucket_id']; ?> map-info" id="<?php echo $ol['bucket_id'] ?>" style="display:none;"><strong><?php echo stripslashes($ol['title']);?></strong><?php echo $own_result;?><br><span class="close" id="<?php echo $ol['bucket_id'] ?>" onclick="popUpclose(<?php echo $ol['bucket_id']?>);"></span></div>',
  ...
  $('.chk').on("click", function(){
	...
	<?php if(isset($succmsg) && $succmsg != '' ){ ?>
	  ...
	  <a href="<?php echo FRONTEND_URL;?>register/updateImg" class="img-upload">Change Image</a>
	  </div>
	  <div class="frnd_list">
	   ...
	   echo stripslashes($travel['type_name'].$val);?></span>
	    ...
	    <div class="selectBox discountType">
              <dl class="select-box">
                <dt> <a href="javascript:void(0);"> <span class="hida">Select Friends to Compare (+)</span>
                  <p class="multiSel"></p>
                  </a> </dt> 
                <dd>
         ...
         <!-- <div class="scrollbar">
		  ...
		  $totalActiveSavings=0;
		    ...
		    id="main_<?php echo $bucket['bucket_id'];?>">
				   ...
				   <a href="javascript:void(0);" onclick='show_bucket_map("<?php echo $bucket['deal_flyer_id'];?>","<?php echo $bucket['type'];?>","","<?php echo $bucket['bucket_status']?>");' data-toggle="modal" class="view-flyer viewOfferLt" data-element="<?php echo $bucket['deal_flyer_id'];?>" data-target="#view-flyer<?php echo $bucket['deal_flyer_id'];?>">
					  <span><?php echo $bucket['title'];?></span>
				   </a>
				   <?php }else{ ?>
				   <a href="javascript:void(0);" onclick='show_bucket_map("<?php echo $bucket['deal_flyer_id'];?>","<?php echo $bucket['type'];?>","","<?php echo $bucket['bucket_status']?>");' data-toggle="modal" class="view-offer viewOfferLt" data-element="<?php echo $bucket['deal_flyer_id'];?>" data-target="#view-offer<?php echo $bucket['deal_flyer_id'];?>">
					  <span><?php echo $bucket['title'];?></span>
				   </a>
				...
				{
				?>
				<span><a href="javascript:void(0);" class="fl" id="review_<?php echo $bucket['bucket_id'];?>" data-id = "<?php echo $bucket['bucket_id'];?>" data-element="<?php echo $bucket['type']; ?>"><img title="Post Review" src="<?php echo FRONTEND_URL;?>images/tick-g.png" alt="" /></a></span>
				<?php
				}
				?>
			...
			<option value="2" <?php if($ownbucket['privacy']=='2'){?> selected="selected" <?php } ?> >Public</option>
		     ...
		     <div class="listRight">				      
			    ...
			    <div class="listIcons">
			      ...
			      <span><a href="javascript:void(0);" class="fl" id="review_<?php echo $ownbucket['bucket_id'];?>" data-id = "<?php echo $ownbucket['bucket_id'];?>"><img title="Post Review" src="<?php echo FRONTEND_URL;?>images/tick-g.png" alt="" /></a></span>
...
<!------------------------- My own item----------------------->
  ...
  <div class="modal fade print_offer_box" id="view-offer" tabindex="-1" role="dialog" aria-hidden="true">
	   ...
	   <div class="modal-content">
		   <div class="modal-header">
			      <h2>Offer Details</h2> <a href="javascript:void(0);" id="print_deal" class="printlink" style="display: none;"></a> 
			      <button class="close" aria-hidden="true" data-dismiss="modal" type="button">&times;</button>
		   </div>
		   <div class="modal-body">
...
<input type="text" name="active_deduct_val" id="active_deduct_val" value="0" />
<input type="text" name="expired_deduct_val" id="expire_deduct_val" value="0" />
<input type="text" name="all_val" id="all_val" value="0" />
<input type="text" name="completed_added_val" id="completed_added_val" value="0" />

<input type="text" name="completed_total_val" id="completed_total_val" value="<?php echo $totalCompletedSavings;?>"/>
<input type="text" name="expired_total_val" id="expired_total_val" value="<?php echo $totalExpireSavings;?>" />
<input type="text" name="active_total_val" id="active_total_val" value="<?php echo $totalActiveSavings;?>" />
<input type="text" name="all_total_val" id="all_total_val" value="<?php echo $totalSavings;?>" />
<input type="text" name="completed_count" id="completed_count" value="<?php echo $cmp_count;?>" />
  ...
  if (vl!= 'My own item' && vl != 'Invited') {
   ...
   $('.btmSaving').css('display','block');
    ...
    if (vl == 'completed') {
      ...
      var alltotalval = parseInt(completed_val)+parseInt(expdeduct_val)+parseInt(actvdeduct_val);
				  ...
				  success: function(msg){
				      ...
				      if(msg >0)
				      {
						  ...
						  if (class_name == 'expired') {
						    ...
						    var expired_savings = parseInt(total_expsavings) - parseInt(res[1]);
						    var exp_single_val = $('#expire_deduct_val').val();
						    var expired_deduct_val = parseInt(exp_single_val) + parseInt(res[1]);
						  ...
						  if (class_name == 'bucketActive') {
						   ...
						   var active_savings = parseInt(total_activesavings) - parseInt(res[1]);
						   var actv_single_val = $('#active_deduct_val').val();
						   var active_deduct_val = parseInt(actv_single_val) + parseInt(res[1]);
						  ...
						  if (class_name == 'all') {
						   ...
						   var all_val = parseInt(all_single_val) + parseInt(res[1]);
						   $('#all_val').val(all_val);
						  }
