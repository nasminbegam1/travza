  ...
  var map;
	...
	position: new google.maps.LatLng('<?php  echo $friend_lat_arr[$k];?>', '<?php echo $friend_long_arr[$k];?>'),
	  ...
	  id: '<?php echo $fl['friend_id']."_".$fl['bucket_id'];?>',
          content: '<img class="shw" id="<?php echo $fl['friend_id']."_".$fl['bucket_id'];?>" onclick="popUpshow(<?php echo $fl['bucket_id']?>);" src="<?php echo $profile_image ?>" style="height:20px;width:20px;"/><div  id="<?php echo $fl['bucket_id'] ?>" style="display:none;"><?php echo "<strong>".stripslashes($fl['title'])."</strong><br>".stripslashes($fl['address']);?></div>',
	    ...
	    <div class="selectBox discountType">
              <dl class="select-box">
				...
				$fl_id = 0;
                            ...
                            ?>
                            <li>  
                                   <input type="checkbox" class="chk" name="type[]" data-fl-id="<?php echo $fl_id;?>" data-element="<?php echo stripslashes($fr['first_name'])." ".stripslashes($fr['last_name']); ?>" value="<?php echo $fr['user_id'];?>" id="<?php echo $fl_id;?>" />
