	...
	<div class="mainPanThree">
		<div class="mainInnThree">
			<div class="tipsPan">
				...
				<?php
				    $class = '';
				    if(is_array($event_user))
				    {
					if(count($event_user) == 1)
					{
					       $class = 'tips_one';
					}
					if(count($event_user) == 2)
					{
					       $class = 'tips_two';
					}
				    }
				    ...
				    if(is_array($event_user))
				    {
					     ...
					     foreach($event_user as $user)
					     {
						     $user_name = stripslashes($user['username']);
						      ...
						      if($image != '' && file_exists(FILE_UPLOAD_ABSOLUTE_PATH."home_user/event/".$image))
						      {
							     $image_path = FILE_UPLOAD_URL."home_user/event/".$image;  
						      }
						      else
						      {
...
$('.view-offer').click(function(){
	 ...
	 $.ajax({			
		  type: "POST",
		  url: "<?php echo FRONTEND_URL; ?>" + "home/view_offer/",
