		...
		if(isset($flyer_list) && is_array($flyer_list) && count($flyer_list) > 0)
		{
			foreach($flyer_list as $flyer)
			{
				...
				$flyer_image = FILE_UPLOAD_URL."flyer/thumb/".$flyer['profile_image']; ?>
							...
							<div class="dealsBoxBottom">
							<h3 data-toggle="modal" class="view-flyer" data-element="<?php echo $flyer['fl_id'];?>" data-target="#view-flyer<?php echo $flyer['fl_id'];?>"><?php echo stripslashes($flyer['flyer_title']);?></h3>
							<ul>
								<li><strong class="location"></strong><span><?php echo stripslashes($flyer['city']);?></span></li>
	...
	<div class="mainPanThree">
		<div class="mainInnThree">
			<div class="tipsPan">
				...
				<?php
				    $class = '';
				    if(is_array($restaurant_user))
				    {
					if(count($restaurant_user) == 1)
					{
					       $class = 'tips_one';
					}
					if(count($restaurant_user) == 2)
					{
					       $class = 'tips_two';
					}
				    }
				...
				<div class="tipsCont <?php echo $class;?>">
				    ...
				    {
					     //pr($restaurant_user);
					     foreach($restaurant_user as $user)
					     {
						      ...
						      if($image != '' && file_exists(FILE_UPLOAD_ABSOLUTE_PATH."home_user/restaurants/".$image))
						      {
							     $image_path = FILE_UPLOAD_URL."home_user/restaurants/".$image;  
						      }
						      else
						      {
